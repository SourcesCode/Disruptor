...
namespace Disruptor.Tests.Example
{
    public class ThreeToOneDisruptor
    {
        ...
        {
            var disruptor = new Disruptor<DataEvent>(new DataEventEventFactory(3), 1024, TaskScheduler.Default);
        ...
        public class DataEventEventFactory : IEventFactory<DataEvent>
        {
            private int _size = 0;
            public DataEventEventFactory(int size)
            {
                _size = size;
            }
            public DataEvent NewInstance()
            {
                return new DataEvent(_size);
            }
